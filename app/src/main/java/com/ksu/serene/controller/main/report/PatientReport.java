...
public class PatientReport extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (Constants.entriesBar != null && Constants.entriesBar.size() > 0 &&
                Constants.xAxisValues != null && Constants.xAxisValues.size() > 0) {
            setChartData(Constants.entriesBar, Constants.xAxisValues);
        } else {
    ...
    private void init() {
        ...
        chartReport = findViewById(R.id.chartReport);
    }
...
//                            locationRecyclerView.setHasFixedSize(true);
    ...
    private void setChartData(List<BarEntry> entriesBar, List<String> xAxisValues) {
        chartReport.setVisibility(VISIBLE);
        ...
        YAxis rightYAxis = chartReport.getAxisRight();
        rightYAxis.setEnabled(false);
        XAxis topXAxis = chartReport.getXAxis();
        ...
        YAxis yAxis = chartReport.getAxisLeft();
        yAxis.setAxisMinimum(0f); // start at zero
        yAxis.setAxisMaximum(3f); // the axis maximum is 100

        chartReport.getXAxis().setValueFormatter(new IndexAxisValueFormatter(xAxisValues));
        chartReport.getDescription().setEnabled(false);

        chartReport.setData(data);

        //off all controls
//        chartReport.setTouchEnabled(false);  //set false if you want off touch events

        chartReport.setHighlightPerTapEnabled(false); // disabled bar selected (another color)
        chartReport.setDragEnabled(true);
        ...
        chartReport.getXAxis().setDrawGridLines(false);
        chartReport.getAxisLeft().setDrawGridLines(false);
        chartReport.getAxisRight().setDrawGridLines(false);
        chartReport.setFitBars(true); // make the x-axis fit exactly all bars

        //settings legend color, count labels, space
        Legend leg = chartReport.getLegend();
        ...
        chartReport.setVisibleXRangeMaximum(7); //show maximum bar count on screen
        chartReport.moveViewToX(7);
        chartReport.animateY(500);
        chartReport.invalidate(); // refresh
    }
